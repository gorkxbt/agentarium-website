...
const GameSimulation = ({ onAgentSelect }: { onAgentSelect?: (agentType: string) => void }) => {
  ...
  return (
    ...
    >
      ...
      {/* Time of Day indicator (non-interactive) */}
      <div className="absolute top-4 right-4 z-10">
        <div className="px-3 py-2 bg-agent-black/60 backdrop-blur-sm rounded-lg border border-white/10 flex items-center">
          <span className="text-yellow-400/90 mr-2">☀️</span>
          <span className="text-white/80 text-xs">Day time</span>
        </div>
      </div>
